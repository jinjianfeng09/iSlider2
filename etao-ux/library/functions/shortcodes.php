...
function hybrid_add_shortcodes() {
	...
	//增加右侧栏的标题
	add_shortcode( 'siderbar-title', 'hybrid_siderbar_title_shortcode' );
	add_shortcode( 'siderbar-author', 'hybrid_siderbar_author_shortcode' );
...
function hybrid_entry_comments_link_shortcode( $attr ) {
	...
	elseif ( 0 == $number )
		$comments_link = '<a class="' . esc_attr( $attr['css_class'] ) . ' no-comment" href="' . get_permalink() . '#respond" title="' . sprintf( esc_attr__( 'Comment on %1$s', 'hybrid-core' ), the_title_attribute( 'echo=0' ) ) . '">' . sprintf( $attr['zero'], number_format_i18n( $number ) ) . '</a>';
	elseif ( 1 == $number )
...
function hybrid_siderbar_author_shortcode() {
	$author = '<span class="author">'. get_the_author_meta( 'display_name' ) . '</span>';
	return  $author;
}
...
function hybrid_siderbar_title_shortcode(){
	global $post;
	
	
   $title = the_title( '<a class="p" href="' . get_permalink() . '" title="' . the_title_attribute( 'echo=0' ) . '" rel="bookmark">', '</a>', false );
	return $title;
...
function hybrid_comment_published_shortcode() {
	$link = sprintf( '<span class="comment-date" title="' . get_comment_date( esc_attr__( 'l, F jS, Y, g:i a', 'hybrid-core' ) ) . '">' . get_comment_date() . '</abbr>', '<abbr class="comment-time" title="' . get_comment_date( esc_attr__( 'l, F jS, Y, g:i a', 'hybrid-core' ) ) . '">' . get_comment_time() . '</span>' );
	return $link;
}
...
function hybrid_comment_author_shortcode( $attr ) {
	...
	$attr = shortcode_atts(
		array(
			'before' => '',
			'after' => '',
			'tag' => 'div' // @deprecated 1.2.0 Back-compatibility. Please don't use this argument.
		),
		$attr
	);
